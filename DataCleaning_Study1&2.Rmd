---
title: "DataCleaning_Study1 & Study 2"
...
- Ppt_Response_Nb: The response given by the participant in terms of number (for the localization task: 1=g, 2=b, 3=h, 4=y; for the categorization task: 1=b, 2=n)

- Ppt_Response: The response given by the participant (localization: g, b, h or y; categorization: b, n)

- Ppt: The name of participant experiment file

- response_id: The number of each participant

- Catego_Key_Response: the theoretical correct response key for the same task with a categorization instruction (b or n)

- Key_Response_Nb: The correct response for the task in terms of number (localization: 1=g, 2=b, 3=h, 4=y; categorization: 1=b, 2=n)
...
- Order: The order of task completion.
...
- Emotion_Type: the emotion  participant have to rate using the same name as in experiment programing ("Colere", "Calme", "Angoisse", "Nervosite", "Joie", "Tension", "Stress", "Deprime", NA )
...
Fear_Score_Pretest: A factorial composite score of 4 anxiety items (Tense,
Anxious, Nervous, Stressed) measured at the beginning of the experiment, using a Bartlett method

Fear_Score: A factorial composite score of 4 anxiety items (Tense,
Anxious, Nervous, Stressed) measured after the experimental block, using a Bartlett method

Evol_Anxiety_Bart: The difference between the anxiety factorial score after the experimental block and at the beginning of the experiment. Higher scores reflect an increasing of the anxiety feelings at the post-measurement in comparison to the pre-measurement (Fear_Score - Fear_Score_Pretest)

Fear_Mean_Pretest: A composite score of 4 anxiety items (Tense, Anxious, Tense,
Stressed) measured at the beginning of the experiment, using a mean calculation rather than a factorial score

Fear_Mean: A composite score of 4 anxiety items (Tense, Anxious, Tense,
Stressed) after the experimental block, using a mean calculation rather than a factorial score

Evol_Anxiety_Mean: The difference between the anxiety mean score after the experimental block and at the beginning of the experiment. Higher scores reflect an increasing of the anxiety feelings at the post-measurement in comparison to the pre-measurement (Fear_Mean - Fear_Mean_Pretest)
...
- Task_Type: Which spatial cueing task participants have to perform:
    - 1 = SpatialCueing_Localization : Participants receive a simple search instruction to complete the spatial cueing task (they have only to locate the target) 
    - 2 = SpatialCueing_Categorization : Participants receive a coumpund search instruction to complete the spatial cueing task (they have to locate AND categorize the target).
    
    
    
- Check_Sound: Did participants hear any sound during the experiment 
  - 1 = Participants did not hear any sound\
  - 2 = Participant heard some sounds

Check_Uncomfort: Did participants hear these sound at an uncomfortable level
  - 1 = Sound level was not uncomfortable\
  - 2 = Sound level was uncomfortable

Check_Volume: Did participants change the volume of their computer after the sound calibration
  - 1 = Participants changed their sound level\
  - 2 = Participants do not change their sound level
  
  Threat_Check_1: Participant rating on the threat perceived by the Screams during the experiment(i.e., "The Screams I heard during the experiment were threatening"). Responses on a 10-likert scale
- 1 = "Not at all" 
- 10 = "Extremely"

Threat_Check_2: Participant rating on the threat perceived by the Screams during the experiment(i.e., "I felt stressed by the possibility of hearing Screams during the task"). Responses on a 10-likert scale
- 1 = "Not at all" 
- 10 = "Extremely"

Threat_Check_3: Participant rating on the threat perceived by the Vocalizations during the experiment(i.e., "The Vocalizations I heard during the experiment were threatening"). Responses on a 10-likert scale
- 1 = "Not at all" 
- 10 = "Extremely"

Threat_Check_4: Participant rating on the threat perceived by the Vocalizations during the experiment(i.e., "I felt stressed by the possibility of hearing Vocalizations during the task"). Responses on a 10-likert scale
- 1 = "Not at all" 
- 10 = "Extremely"

Subjective_SES: Participant rating on the Mac Arthur scale (Adler, 2000) measuring subjective social status. Response on a 10-Likert scale
- 1 = "Not at all" 
- 10 = "Extremely"
...
Country: The country where participant live in

Studies: The highest level of education achieved by the participants
- 0 = No qualification
- 1 = Secondary schools certificate
- 2 = CAP (certificate of professional competence) or BEP (Professional Studies Certificate)
- 3 = Baccalaureate
- 4 = BAC+1
- 5 = BAC+2
- 6 = BAC+3 
...
#   mutate(ID_Yapper_1 = "####")
# 
# write.csv(df_Anonymous, file = "Data/data_anonymous.csv")

tmp2 <- data <- read_csv("Data/data_anonymous.csv")%>%
    select(-"...1")
...
write.csv(df_Final, file = "Data/Transformed_Data/Final_df_Threat_SpatialCueing.csv")
write.csv(df_Final, file = "Data/Transformed_Data/Final_df_Threat_SpatialCueing_NotAnonymous.csv")
